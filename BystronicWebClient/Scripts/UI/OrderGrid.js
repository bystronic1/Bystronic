function OrderGrid(scope, gridID, onGridRowDouldClicked) {
    ...
    getColumnFor = function (columnName) {
        switch (columnName) {
            ...
            case 'ToolingPrice':
                return {
                    name: 'ToolingPrice', width: 85, align: 'center', editable: false, autoResizing: { minColWidth: 80 }, sorttype: 'float', formatter: 'currency', search: false,
                    formatoptions: { prefix: '$', suffix: '', thousandsSeparator: ',', decimalPlaces: 2, defaultValue: '' }
                };
            default:
                return '';
    ...
    this.filterData = function (data, filter) {
        ...
        $.each(data, function (index, order) {
            ...
            _order.ToolingPrice = _order.TemplateID == 3 ? self.getToolingPrice(_order) : null;
    ...
    this.getToolingPrice = function (order) {
        let toolingPrice = 0;
        $.each(order.OrderItems, function (index, orderItem) {
            toolingPrice += orderItem.ToolingValue;
        });
